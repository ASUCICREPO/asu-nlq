...
def health_check():
    """
    Simple health check function for monitoring and testing.
    
    Validates that all required components are importable and
    environment variables are properly configured.
    
    Returns:
        dict: Health status with component validation results
    """
    health_status = {
        "status": "healthy",
        "components": {},
    ...
    try:
        ...
        import boto3
        import urllib3
        health_status["components"]["boto3"] = "available"
        health_status["components"]["urllib3"] = "available"
        
        # Test AWS service clients
        try:
            amplify_client = boto3.client('amplify')
            health_status["components"]["amplify_client"] = "initialized"
        except Exception as e:
            health_status["components"]["amplify_client"] = f"error: {str(e)}"
            health_status["status"] = "degraded"
        
        try:
            s3_client = boto3.client('s3')
            health_status["components"]["s3_client"] = "initialized"
        except Exception as e:
            health_status["components"]["s3_client"] = f"error: {str(e)}"
            health_status["status"] = "degraded"
        
        # Test environment variables (imported from constants)
        try:
            from constants import AMPLIFY_APP_NAME, FRONTEND_BUCKET_NAME, FRONTEND_FOLDER_NAME
            health_status["components"]["environment_variables"] = "validated"
        except Exception as e:
            health_status["components"]["environment_variables"] = f"error: {str(e)}"
